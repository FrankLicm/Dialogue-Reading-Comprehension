...
def gen_examples(x1, x2, l, y1, y2, qmask, dmask, batch_size):
    ...
    for minibatch in minibatches:
        ...
        all_ex.append((mb_x1, mb_x2, mb_l, mb_y1, mb_y2, mb_qmask, mb_dmask))
    return all_ex

def pre_shuffle(x1, x2, l, y1, y2, qmask, dmask):
    combine = list(zip(x1, x2, l, y1, y2, qmask, dmask))
    np.random.shuffle(combine)
    x1, x2, l, y1, y2, qmask, dmask = zip(*combine)
    return list(x1), list(x2), np.array(l), list(y1), list(y2), np.array(qmask), np.array(dmask)
...
def eval_acc(any_model, all_examples, max_d, max_q, max_s):
    acc = 0
    n_examples = 0
    for x1, x2, l, y1, y2, q_mask, d_mask in all_examples:
        newx1 = []
...
def cnn_lstm_DA(args):
    ...
    if args.model_to_run == 'cnn_lstm':
        cnn_model = CNN_LSTM_Model('CNN_LSTM_Model', num_labels, vocab_size, args.embedding_size, max_d, max_q, max_s,
            nb_filters_utterance=args.utterance_filters, nb_filters_query=args.query_filters, learning_rate=args.learning_rate,
            dropout=args.dropout, nb_hidden_unit=args.hidden_size)
    if args.model_to_run == 'lstm':
        cnn_model = LSTM_Model('LSTM_Model', num_labels, vocab_size, args.embedding_size, max_d, max_q, max_s,
                                   nb_filters_utterance=args.utterance_filters, nb_filters_query=args.query_filters,
    ...
    dev_x1, dev_x2, dev_l, dev_y1, dev_y2, dev_qmask, dev_dmask = tools.vectorize(dev_examples, word_dict, entity_dict,
                                                                                  max_d, max_q, max_s)
    assert len(dev_x1) == num_dev
    
    all_dev = gen_examples(dev_x1, dev_x2, dev_l, dev_y1, dev_y2, dev_qmask, dev_dmask, args.batch_size)
    ...
    train_x1, train_x2, train_l, train_y1, train_y2, train_qmask, train_dmask = tools.vectorize(train_examples, word_dict, entity_dict, max_d, max_q, max_s)
    assert len(train_x1) == num_train
    
    train_x1, train_x2, train_l, train_y1, train_y2, train_qmask, train_dmask = pre_shuffle(train_x1, train_x2, train_l, train_y1, train_y2, train_qmask, train_dmask)
    start_time = time.time()
    n_updates = 0
    all_train = gen_examples(train_x1, train_x2, train_l, train_y1, train_y2, train_qmask, train_dmask, args.batch_size)

    for epoch in range(args.nb_epoch):
        np.random.shuffle(all_train)
        
        for idx, (mb_x1, mb_x2, mb_l, mb_y1, mb_y2, mb_qmask, mb_dmask) in enumerate(all_train):
            ...
            hist = cnn_model.fit(newx1+[np.array(mb_x2)]+[np.array(mb_l)] +[np.array(mb_qmask)]+[np.array(mb_dmask)],
...
if __name__ == '__main__':
    ...
    model_dict = ['cnn_lstm', 'cnn_lstm_DA', 'lstm']
