...
def vectorize(examples, word_dict, entity_dict, max_d, max_q, max_s, verbose=True):
    ...
    for idx, (d, q, a) in enumerate(zip(examples[0], examples[1], examples[2])):
        ...
        if a[0] == "":
            ...
            in_y1.append(entity_dict[a[0]])
        if a[1] == "":
            in_y2.append(0)
        else:
            in_y2.append(entity_dict[a[1]])
        in_qmask[idx, :min(len(q_words), max_q)] = 1.0
        
    return in_x1, in_x2, in_l, in_y1, in_y2, in_qmask, in_dmask
...
def bert_vectorize(examples, token_dict, entity_dict):
    ...
    for idx, (d, q, a) in enumerate(zip(e1, e2, e3)):
        ...
        for token in tokens:
            ...
            elif token == "placeholder1":
                token_input.append(len(entity_dict) + 3)
            elif token == "placeholder2":
                token_input.append(len(entity_dict) + 4)
            else:
        ...
        if a[0] == "":
            ...
            index = int(''.join(filter(str.isdigit, a[0])))
            in_y1.append(index+1)
        if a[1] == "":
            in_y2.append(0)
        else:
            index = int(''.join(filter(str.isdigit, a[1])))
            in_y2.append(index+1)
    return token_inputs, seg_inputs, in_l, in_y1, in_y2

def bert_decoder_vectorize(examples, token_dict, entity_dict):
    ...
    for idx, (d, q, a) in enumerate(zip(examples[0], examples[1], examples[2])):
        ...
        d_words.append(np.zeros(768))
        segment_len = len(tokens)
        for u in range(min(len(d), 20)):
            u_words = d[u].split()
            tokens += u_words[0:min(len(u_words), 19)]
            for j in range(min(len(u_words), 19)):
                if u_words[j] in entity_dict:
                    ...
                    d_words.append(np.zeros(768))
                    en_d.append(np.zeros(nb_class))
            for j in range(19 - min(len(u_words), 19)):
                d_words.append(np.zeros(768))
                en_d.append(np.zeros(nb_class))
                tokens.append('[PAD]')
        for u in range(20-min(len(d), 20)):
            for j in range(19):
                tokens.append('[PAD]')
                en_d.append(np.zeros(nb_class))
                d_words.append(np.zeros(768))
...
def load_jsondata(in_file, redundent_1, redundent_2, stopwords_file):
    ...
    for sample in samples:
        ...
        answer1 = entity_dict[answerss[0]] if answerss[0] in entity_dict else ""
        answer2 = entity_dict[answerss[1]] if answerss[1] in entity_dict else ""
        re_document = []
