...
def gen_examples(x1, x2, match, l, y1, y2, qmask, dmask, batch_size):
    ...
    for minibatch in minibatches:
        ...
        all_ex.append((mb_x1, mb_x2, mb_match, mb_l, mb_y1, mb_y2, mb_qmask, mb_dmask))
    return all_ex


def pre_shuffle(x1, x2, l, y1, y2, qmask, dmask, match):
    combine = list(zip(x1, x2, l, y1, y2, qmask, dmask, match))
    np.random.shuffle(combine)
    x1, x2, l, y1, y2, qmask, dmask, match = zip(*combine)
    return list(x1), list(x2), np.array(l), list(y1), list(y2), np.array(qmask), np.array(dmask), list(match)
...
def eval_acc(any_model, all_examples, max_d, max_q, max_s):
    acc = 0
    n_examples = 0
    for x1, x2, match, l, y1, y2, q_mask, d_mask in all_examples:
        newx1 = []
        ...
        predictions = any_model.predict_classes(
            newx1 + new_match + [np.array(x2)] + [np.array(l)]
...
def cnn_lstm_UA_DA(args):
    ...
    dev_x1, dev_x2, dev_l, dev_y1, dev_y2, dev_qmask, dev_dmask = tools.vectorize(dev_examples, word_dict, entity_dict, max_d, max_q, max_s)
    ...
    all_dev = gen_examples(dev_x1, dev_x2, dev_matchscore, dev_l, dev_y1, dev_y2, dev_qmask, dev_dmask, args.batch_size)
    ...
    train_x1, train_x2, train_l, train_y1, train_y2, train_qmask, train_dmask, train_matchscore = pre_shuffle(train_x1, train_x2, train_l, train_y1, train_y2, train_qmask, train_dmask, train_matchscore)
    start_time = time.time()
    n_updates = 0
    all_train = gen_examples(train_x1, train_x2, train_matchscore, train_l, train_y1, train_y2, train_qmask, train_dmask, args.batch_size)

    for epoch in range(args.nb_epoch):
        np.random.shuffle(all_train)
        
        for idx, (mb_x1, mb_x2, mb_match, mb_l, mb_y1, mb_y2, mb_qmask, mb_dmask) in enumerate(all_train):
            ...
            hist = cnn_model.fit(newx1+new_match+[np.array(mb_x2)]+[np.array(mb_l)]
                +[np.array(mb_qmask)]+[np.array(mb_dmask)], [np.array([n for n in mb_y1]), np.array([n for n in mb_y2])],
