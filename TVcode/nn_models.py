...
from keras_bert.layers.pooling import MaskedGlobalMaxPool1D
...
class CNN_LSTM_UA_DA_Model(BaseModel):
    ...
    def build_model(self):
        ...
        classes2 = Dense(units=self.nb_classes, activation='softmax')(merged_vectors)
        # masking 
        classes_normalized = Lambda(self.masking_lambda, output_shape=(self.nb_classes,))([classes, inputs[-3]])
        classes_normalized2 = Lambda(self.masking_lambda, output_shape=(self.nb_classes,))([classes2, inputs[-3]])
        model = Model(inputs=inputs, outputs=(classes_normalized, classes_normalized2))
    ...
    def predict_classes(self, x, y_masks):
        ...
        predictions_masked = [predictions[0] * y_masks, predictions[1] * y_masks]
        classes1 = [np.argmax(i) for i in predictions_masked[0]]
        classes2 = [np.argmax(i) for i in predictions_masked[1]]
...
class Bert_Model(BaseModel):
    ...
    def entity_position_lambda(self, x):
        bert_output, entity_positions = x[0], x[1]
        entity_positions = bert_output * entity_positions  # 512
        masked_entity_positions = K.clip(entity_positions, 1e-7, 1.0 - 1e-7)
        return masked_entity_positions

    def find_entity_lambda(self, x):
        softmaxed_tokens, en_d = x[0], x[1]
        # softmaxed_tokens is (?, 512), en_d is (?, 512, 17), we want (?, 17)
        p = K.argmax(softmaxed_tokens, axis=-1)
        q = tf.expand_dims(p, axis=-1)
        k = tf.cast(q, tf.int32)
        z = tf.batch_gather(en_d, k)
        output = tf.squeeze(z, axis=1)
    ...
    def get_model(self,
        ...
        #pool_layer = MaskedGlobalMaxPool1D(name='Pooling')(transformed)
        first_token_tensor = Lambda(lambda y: K.squeeze(y[:, 0:1, :], axis=1))(transformed)
        classes1 = Dense(units=self.nb_classes,
                        use_bias=True,
                        kernel_initializer=truncated_normal(stddev=0.02),
                        activation='softmax')(first_token_tensor)
        classes2 = Dense(units=self.nb_classes,
                        use_bias=True,
                        kernel_initializer=truncated_normal(stddev=0.02),
                        activation='softmax')(first_token_tensor)
        classes_normalized1 = Lambda(self.masking_lambda, output_shape=(self.nb_classes,))(
            [classes1, model_inputs[-1]])
        classes_normalized2 = Lambda(self.masking_lambda, output_shape=(self.nb_classes,))(
            [classes2, model_inputs[-1]])
        model = Model(inputs=model_inputs, outputs=(classes_normalized1, classes_normalized2))
        opt = Adam(lr=self.learning_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-6, decay=0.01)
        model.compile(loss='sparse_categorical_crossentropy', optimizer=opt)
        return model
